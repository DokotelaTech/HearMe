...
//  Handles SOS alerts + session-start email notifications
// ============================================================

const User        = require('../database/models/User');   // adjust path if different
...
/**
 * Email sent to EACH THERAPIST when a client triggers SOS.
 * @param {string} therapistName   - therapist's first name
 * @param {string} clientName      - client's anonymous / display name
 */
...
<head>
  ...
  <style>
    ...
    .btn { display:inline-block; background:#c0392b; color:#ffffff !important; text-decoration:none; padding:14px 36px; border-radius:8px; font-size:16px; font-weight:700; letter-spacing:0.5px; }
...
<body>
  <div class="wrapper">
    ...
    <div class="footer">
      © ${new Date().getFullYear()} HearMe · You are receiving this because you are a registered HearMe therapist.
    </div>
  </div>
</body>
</html>`;
...
/**
 * Email sent to the CLIENT when a therapist starts a session.
 * @param {string} clientName      - client's display/anonymous name
 * @param {string} therapistName   - therapist's full name
 */
...
<head>
  ...
  <style>
    ...
    .btn-wrap { text-align:center; margin:28px 0 8px; }
    .btn { display:inline-block; background:#1a7f64; color:#ffffff !important; text-decoration:none; padding:14px 36px; border-radius:8px; font-size:16px; font-weight:700; letter-spacing:0.5px; }
    .footer { background:#f4f4f7; padding:20px 40px; text-align:center; color:#999; font-size:12px; }
  </style>
</head>
<body>
  <div class="wrapper">
    ...
    <div class="footer">
      © ${new Date().getFullYear()} HearMe · You're receiving this because you activated an SOS on HearMe.
    </div>
  </div>
</body>
</html>`;
...
/**
 * Email sent to the THERAPIST confirming the session they just started.
...
<head>
  ...
  <style>
    ...
    .btn-wrap { text-align:center; margin:28px 0 8px; }
    .btn { display:inline-block; background:#2c3e50; color:#ffffff !important; text-decoration:none; padding:14px 36px; border-radius:8px; font-size:16px; font-weight:700; }
    .footer { background:#f4f4f7; padding:20px 40px; text-align:center; color:#999; font-size:12px; }
  </style>
</head>
<body>
  <div class="wrapper">
    ...
    <div class="footer">
      © ${new Date().getFullYear()} HearMe · Thank you for making a difference.
    </div>
  </div>
</body>
</html>`;
...
async function triggerSOS(req, res) {
    try {
        ...
        if (!therapists.length) {
            return res.status(503).json({ message: 'No therapists available right now. Please call emergency services.' });
        }

        const clientName = client.anonymousName || client.username || 'A HearMe user';
        ...
        return res.status(500).json({ message: 'SOS activation failed. Please call emergency services immediately.' });
    }
}
...
async function notifySessionStarted(req, res) {
    try {
        const therapist = req.user; // set by verifyToken middleware
        ...
        const therapistName = `${therapist.firstName || ''} ${therapist.lastName || ''}`.trim() || 'Your HearMe Therapist';
