document.addEventListener("DOMContentLoaded", () => {
    ...
    async function openMessage(id) {
        ...
        const doReply = async () => {
            ...
            // Use therapist identifier from localStorage (required for inbox rendering consistency)
            const therapistName = (function () {
                try {
                    const u = JSON.parse(localStorage.getItem('user'));
                    if (u && u.identifier) return u.identifier.toLowerCase().trim();
                } catch {}
                const id = localStorage.getItem('userIdentifier') || localStorage.getItem('identifier');
                if (id) return String(id).toLowerCase().trim();
                return null;
            })() || document.getElementById('therapistName')?.innerText || 'Dr. Sarah Johnson';
