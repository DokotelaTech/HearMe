(() => {
    ...
    function closeComposer() {
        ...
        if(selectedGifBox) selectedGifBox.innerHTML = '';
        tabBtns.forEach(b => b.classList.remove('active'));
        if(tabBtns.length > 0) tabBtns[0].classList.add('active');
        postType = 'struggle';
        if(textarea) textarea.placeholder = "Share what you're going through... You're not alone.";
    }

    if(simpleInput) simpleInput.addEventListener('click', openComposer);
    ...
    tabBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if(textarea) textarea.placeholder = postType === 'struggle'
    ...
    async function loadGiphyKey() {
        try {
            ...
            console.error("Failed to load Giphy key", e);
        }
    }
    ...
    async function searchGifs() {
        ...
        try {
            ...
            gifResults.innerHTML = `<p class="gif-state error">Failed to load GIFs.</p>`;
        }
    }

    document.getElementById('open-gif-search')?.addEventListener('click', () => {
        if(gifPanel) gifPanel.style.display = gifPanel.style.display === 'none' ? 'block' : 'none';
    });
    ...
    // ==========================================
    // REPORT MODAL LOGIC (UPDATED FOR SAFTEY)
    // ==========================================
    ...
    function openReportModal(postId, btn) {
        ...
        // Reset custom category buttons if they exist
        document.querySelectorAll('#reportModal .cat-btn').forEach(c => {
            c.classList.remove('active');
            c.style.backgroundColor = 'white';
            c.style.borderColor = '#e5e7eb';
        });

        const customReason = document.getElementById('reportCustomReason');
        ...
        if(modal) {
            ...
            console.error("Report modal element not found in HTML!");
        }
    }
    ...
    async function submitReport() {
        ...
        // Check for standard Radio Buttons first
        const selectedRadio = document.querySelector('input[name="reportReason"]:checked');
        if (selectedRadio) {
            category = selectedRadio.value;
        } 
        // Fallback: Check for custom Category Buttons (cat-btn)
        else {
            const activeCatBtn = document.querySelector('#reportModal .cat-btn.active');
            if (activeCatBtn) {
                category = activeCatBtn.getAttribute('data-type') || activeCatBtn.textContent.trim();
            }
        }

        const customReasonInput = document.getElementById('reportCustomReason');
        ...
        const finalCategory = (category.toLowerCase() === 'other' && customReason)
            ? `Other: ${customReason}`
            : category;

        const submitButton = document.querySelector('#reportModal button[onclick="submitReport()"]') || document.activeElement;
        ...
        try {
            const response = await fetch('/api/reports', {
                ...
                body: JSON.stringify({
                    ...
                    description: customReason || finalCategory,
            ...
            if (response.ok) {
                ...
                if (reportTargetBtn) {
                    ...
                    reportTargetBtn.innerHTML = '<i data-lucide="flag"></i> Reported';
                    if (typeof lucide !== 'undefined') lucide.createIcons();
                }
            } else {
                const data = await response.json();
    ...
    // Attach custom category button listeners for the modal
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        // Wire up Report Modal Background Click
        ...
        // Wire up custom category buttons inside the report modal (if used)
        const catBtns = document.querySelectorAll('#reportModal .cat-btn');
        catBtns.forEach(btn => {
    ...
    // Make these globally accessible so inline HTML onclick="" works
